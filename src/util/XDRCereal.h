...
cereal_override(cereal::JSONOutputArchive& ar,
                const xdr::xarray<diamnet::Hash, N>& s, const char* field)
{
    std::vector<std::string> tmp;
    for (auto const& h : s)
    {
        tmp.emplace_back(
            diamnet::binToHex(diamnet::ByteSlice(h.data(), h.size())));
    }
...
void cereal_override(cereal::JSONOutputArchive& ar, const diamnet::Asset& s,
                     const char* field);
